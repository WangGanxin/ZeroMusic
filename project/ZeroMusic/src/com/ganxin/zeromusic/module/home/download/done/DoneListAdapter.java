...
/**
 * 
 * @Description 下载完成适配器，该适配器与LocalMusicListAdapter大部分相同，数据库操作不一致
 * @author ganxin
 * @date Sep 5, 2015
...
public class DoneListAdapter extends BaseAdapter{
	 ...
	 * @param cursor 数据源
	 */
	public DoneListAdapter(Context context, Cursor cursor) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.detail.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				String duration = music.getDuration() / 60.0f + "";
					   duration = duration.substring(0, duration.length()<4?duration.length():4) + " 分";
	...
	private void delDialog(int position){
		 ...
		 btnConfirm.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View v) {
				...
				int musicId=dBHelper.queryDownloadByLocalID(localId);
				
				String artist=cursor.getString(cursor.getColumnIndex(DbConstant.LOCAL_ARTIST));
				String musicPath=cursor.getString(cursor.getColumnIndex(DbConstant.LOCAL_PATH));
				String lrcPath=musicPath.replace(".mp3", ".lrc");
				
				//同时删除文件
				if(option.isChecked()){					
					int i= dBHelper.delLocal(localId);
					       dBHelper.delFav(localId);
					       dBHelper.delArtist(artist);
					       dBHelper.delDownloadFromID(musicId);
				...
				//只删除本地数据库
					...
					customDialog.dismiss();
					int i= dBHelper.delDownloadFromID(musicId);
					if(i>0)
						ToastHelper.show(context,R.string.del_music_success);						
					else
						ToastHelper.show(context,R.string.del_music_failed);										
				...
				//以下操作重新刷新listview
				cursor = dBHelper.queryDownloadFromLocal();
				if (cursor != null) {
					popCheckStatus = new ArrayList<Boolean>();
