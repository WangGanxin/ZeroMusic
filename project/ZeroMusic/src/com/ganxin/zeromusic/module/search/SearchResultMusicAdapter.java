...
public class SearchResultMusicAdapter extends BaseAdapter {
	...
	private ArrayList<Song> list; // 数据源
	private String keyword; // 关键字
	private Dialog dialog;
	private DownloadManager downloadManager;

	public SearchResultMusicAdapter(Context context, ArrayList<Song> list,
			String keyword) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(context).inflate(
					R.layout.search_music_listview_item, null);

			viewHolder.musicName = (TextView) convertView
					.findViewById(R.id.search_song_listview_item_song);
			viewHolder.musicArtist = (TextView) convertView
					.findViewById(R.id.search_song_listview_item_artist);
			viewHolder.downloadBtn = (ImageButton) convertView
					.findViewById(R.id.search_song_listview_item_download);
			viewHolder.contentLayout=(RelativeLayout) convertView
					.findViewById(R.id.search_song_listview_item_layout);
		...
		final Song mySong = list.get(position);

		// 设置数据
		String song = mySong.getSongname();
		// 歌名
		if (song.contains(keyword)) {
			SpannableString styledText = new SpannableString(song);

			styledText.setSpan(new TextAppearanceSpan(context,
					R.style.keywordStyle), song.indexOf(keyword),
					song.indexOf(keyword) + keyword.length(),
					Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

			viewHolder.musicName.setText(styledText,
					TextView.BufferType.SPANNABLE);
		} else {
			viewHolder.musicName.setText(song);
		}

		// 歌手
		String artist = mySong.getArtistname();
		if (artist.contains(keyword)) {
			SpannableString styledText = new SpannableString(artist);

			styledText.setSpan(new TextAppearanceSpan(context,
					R.style.keywordStyle), artist.indexOf(keyword),
					artist.indexOf(keyword) + keyword.length(),
					Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

			viewHolder.musicArtist.setText(styledText,
					TextView.BufferType.SPANNABLE);
		} else {
			viewHolder.musicArtist.setText(artist);
		}
